  ...
  const getAllAppointments = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:8000/api/v1/instructorAppointmentAtHome/postAppointment",
  ...
  const addAppointment = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:8000/api/v1/instructorAppointmentAtHome/createService/centerVisit",
        formData,
        {
          withCredentials: true,
